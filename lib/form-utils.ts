...
export const createMinLengthRule = (minLength: number, message: string): ValidationRule => ({
  validate: (value: string) => (value.length >= minLength ? null : message),
})

export const createMaxLengthRule = (maxLength: number, message: string): ValidationRule => ({
  validate: (value: string) => (value.length <= maxLength ? null : message),
})

export const createRegexRule = (regex: RegExp, message: string): ValidationRule => ({
...
export const formatFieldError = (error: FieldError | undefined, fieldName: string): string => {
  if (!error) return ""
...
export const prepareFormSubmission = (data: Record<string, any>, formType: string) => {
...
export const manageFocus = (formRef: React.RefObject<HTMLFormElement>) => {
  ...
  const focusNextField = (currentFieldName: string, fieldOrder: string[]) => {
    ...
    if (currentIndex >= 0 && currentIndex < fieldOrder.length - 1) {
      ...
      const nextField = formRef.current?.querySelector(`[name="${nextFieldName}"]`) as HTMLElement
      if (nextField) {
        nextField.focus()
      }
    }
