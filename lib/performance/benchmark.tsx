...
export class PerformanceBenchmark {
  ...
  private logResult(result: BenchmarkResult): void {
    ...
    console.log(
      `📊 Memory Delta: ${this.formatBytes(result.memory.after - result.memory.before)}`,
    );
  }
  ...
  compareResults(name1: string, name2: string): void {
    ...
    const timeDiff =
      ((result2.averageTime - result1.averageTime) / result1.averageTime) * 100;
    const memoryDiff =
      ((result2.memory.peak - result1.memory.peak) / result1.memory.peak) * 100;

    console.log(
      `⏱️  Time Difference: ${timeDiff > 0 ? "+" : ""}${timeDiff.toFixed(2)}%`,
    );
    console.log(
      `💾 Memory Difference: ${memoryDiff > 0 ? "+" : ""}${memoryDiff.toFixed(2)}%`,
    );
...
export const runFormBenchmarks = async () => {
  ...
  await benchmark.benchmark(
    "Form Validation",
    async () => {
      const { virtualOfficeFormSchema } = await import(
