...
export function sanitizeAnalyticsEvent(event: AnalyticsEvent): Partial<AnalyticsEvent> {
  return {
...
export function aggregateEventsByType(events: AnalyticsEvent[]): Record<string, number> {
...
export function aggregateEventsByForm(events: AnalyticsEvent[]): Record<string, number> {
  return events.reduce(
    (acc, event) => {
...
export function calculateConversionRate(starts: number, completions: number): number {
