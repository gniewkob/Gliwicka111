...
export async function dismissCookieBanner(page: Page) {
  ...
  const acceptAll = page.getByRole("button", {
    name: /akceptuj.*|accept.*all/i,
  });
  if (await acceptAll.isVisible().catch(() => false)) {
    await acceptAll.click();
    return;
  }
  const necessaryOnly = page.getByRole("button", {
    name: /tylko.*niezbędne|necessary\s+only/i,
  });
...
export async function closeGdprModalIfPresent(page: Page) {
  ...
  if (await dialog.isVisible().catch(() => false)) {
    ...
    const accept = dialog.getByRole("button", {
      name: /akceptuj|accept|allow|ok|zamknij|close/i,
    });
